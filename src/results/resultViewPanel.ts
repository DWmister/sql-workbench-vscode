...
function renderResultsHtml(webview: vscode.Webview): string {
  ...
  return [
    ...
    '    .column-name, .column-comment { display: block; overflow: hidden; text-overflow: ellipsis; }',
    '    .column-comment { margin-top: 2px; color: var(--muted); font-size: 0.9em; font-weight: 400; }',
...
function getClientScript(): string {
  return [
    ...
    'function renderTable(resultIndex, result, rows, values) { return "<div class=\\"table-wrap\\"><table><thead><tr>" + result.columns.map(renderColumnHeader).join("") + "</tr></thead><tbody>" + rows.map((row, rowIndex) => "<tr>" + row.map((cell, columnIndex) => renderCell(resultIndex, rowIndex, columnIndex, cell, values[rowIndex] ? values[rowIndex][columnIndex] : cell)).join("") + "</tr>").join("") + "</tbody></table></div>"; }',
    'function renderColumnHeader(column) { const comment = String(column && column.comment !== undefined ? column.comment : "").trim(); const title = comment ? column.name + "\\n" + comment : column.name; return "<th title=\\"" + escapeAttribute(title) + "\\"><span class=\\"column-name\\">" + escapeHtml(column.name) + "</span>" + (comment ? "<span class=\\"column-comment\\">" + escapeHtml(comment) + "</span>" : "") + "</th>"; }',
