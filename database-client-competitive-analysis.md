...
| 表结构信息 | DDL Hover 简版 | **超出计划**：表属性增加 Columns / DDL，支持三类数据库完整 DDL、语法着色、复制、刷新、重试 | 完整表 DDL 归档到 v0.2；字段排序在 v0.2.7 交付；Hover 保持轻量摘要 |
| 结果字段注释 | 未单列 | **已交付**：可靠来源字段在结果表头显示注释 | 常规字段注释归档到 v0.2；多表通配符注释后移到 v0.3.x |
