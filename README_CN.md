...
# 3. 从 VS Code 启动扩展
# 在 VS Code 中按 F5，或使用 Extension Development Host。
```
...
`npm run screenshots` 会用 headless Chrome 渲染 README 图片。如果 Chrome 不在默认 macOS 路径，可设置 `CHROME_PATH`：
...
README 截图由 `npm run screenshots` 生成，这样 UI 调整后可以重复刷新文档图片。
